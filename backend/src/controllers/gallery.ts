...
galleryRouter.get("/hidden-trail", async (ctx) => {
  try {
    ...
    for (const row of data ?? []) {
      const r = row as { id: string; user_id: string; level_id: string; storage_path: string; created_at: string; visibility: string; qr_levels: Array<{ level_number: number }> | null };
      ...
      photos.push({
        id: r.id,
        url,
        display_name: deriveDisplayName((profile as { full_name: string | null } | null)?.full_name, mode),
