...
export type PhotoValidationResult =
  | { ok: true; optimized: Buffer; width: number; height: number }
...
export function buildStoragePath(gameId: string, userId: string, levelId: string): string {
  return `hidden-trail/${gameId}/${userId}/${levelId}/${randomUUID()}.webp`;
}

export async function uploadPhoto(supabase: SupabaseClient, buffer: Buffer, path: string) {
...
export async function deletePhotoObject(supabase: SupabaseClient, path: string) {
  const { error } = await supabase.storage.from(PHOTO_BUCKET).remove([path]);
  if (error) throw error;
}

export async function createPhotoSignedUrl(supabase: SupabaseClient, path: string, expiresIn = 3600) {
  const { data, error } = await supabase.storage.from(PHOTO_BUCKET).createSignedUrl(path, expiresIn);
  if (error || !data) return null;
  return data.signedUrl;
}

export function isAdmin(user: { id: string; email: string; role: string }, configuredAdminEmail?: string) {
