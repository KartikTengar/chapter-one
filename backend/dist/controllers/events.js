...
eventsRouter.get('/', async (ctx) => {
    try {
        ...
        if (ctx.query.category) {
            query = query.eq('category', ctx.query.category);
        }
        if (ctx.query.q) {
            const searchTerm = ctx.query.q.trim();
            if (searchTerm) {
                query = query.or(`title.ilike.%${searchTerm}%,description.ilike.%${searchTerm}%,category.ilike.%${searchTerm}%,location.ilike.%${searchTerm}%`);
            }
        }
        // Apply pagination
