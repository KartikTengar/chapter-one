...
trailRouter.get("/stats", async (ctx) => {
    ...
    const events = [
        ...completionRows.map((c) => ({ kind: "success", at: c.answered_at ?? new Date(0).toISOString() })),
        ...scanRows.filter((s) => s.result === "answer_incorrect").map((s) => ({ kind: "wrong", at: s.created_at })),
...
trailRouter.get("/replay", async (ctx) => {
    ...
    for (const c of (completions.data ?? [])) {
        ...
        const photo = (photos.data ?? []).find((p) => p.level_id === c.level_id);
        timeline.push({
            type: "marker",
...
trailRouter.get("/achievements", async (ctx) => {
    ...
    const list = (allAchievements.data ?? []).map((a) => {
        ...
        return {
            ...
            earned_at: (userAchievements.data ?? []).find((u) => u.achievement_id === ach.id)?.earned_at ?? null,
        };
    });
 ...
 */
async function evaluateAchievements(admin, gameId, userId) {
    try {
        const game = await resolveHiddenTrailGame(admin);
        if (!game)
            return;
        ...
        const wrongCount = (scanLogs.data ?? []).filter((s) => s.result === "answer_incorrect").length;
        ...
        const grant = async (code) => {
            ...
            await admin
                ...
                .upsert({ user_id: userId, achievement_id: achievementId, game_id: gameId }, { onConflict: "user_id,achievement_id,game_id", ignoreDuplicates: true });
        };
        const isComplete = completionCount >= (totalLevelCount ?? 0) && (totalLevelCount ?? 0) > 0;
        const perfect = isComplete && wrongCount === 0;
        const firstCompletion = (allCompletions.data ?? [])[0];
        const photoCount = (photoRows.data ?? []).length;
        if (isComplete) {
            await grant("FINISHER");
            await grant("EXPLORER");
            if (perfect)
                await grant("PERFECT_TRAIL");
        }
        if (firstCompletion && firstCompletion.user_id === userId)
            await grant("FIRST_BLOOD");
        if (photoCount >= 5)
            await grant("PHOTO_HUNTER");
    }
    catch {
