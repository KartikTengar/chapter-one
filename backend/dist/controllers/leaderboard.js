...
async function getDisplayName(admin, userId) {
    const { data } = await admin.from('profiles').select('full_name').eq('id', userId).single();
    return data?.full_name ? String(data.full_name) : 'Student';
}
...
leaderboardRouter.get('/master', async (ctx) => {
    try {
        ...
        if (htErr)
            throw htErr;
        const gameResults = (gameResultsRaw ?? []);
        const hiddenResults = (hiddenResultsRaw ?? []);
...
leaderboardRouter.get('/games/:slug', async (ctx) => {
    ...
    try {
        ...
        if (slug === 'hidden-trail') {
            ...
            const gameId = gameIdRes.data?.id;
            if (gameId) {
