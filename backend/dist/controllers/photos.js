...
import { PHOTO_BUCKET, validateAndOptimizeImage, buildStoragePath, uploadPhoto, deletePhotoObject, createPhotoSignedUrl, } from '../services/photos.js';
function getAdmin() {
...
photoRouter.get("/album", async (ctx) => {
    ...
    for (const row of data ?? []) {
        ...
        photos.push({
            id: r.id,
            level_id: r.level_id,
            level_number: r.qr_levels?.[0]?.level_number ?? null,
            title: r.qr_levels?.[0]?.title ?? null,
...
photoRouter.patch("/photo/:id", async (ctx) => {
    ...
    if (body.data.visibility !== undefined) {
        ...
        if (body.data.visibility !== "private")
            updates.photo_consent_granted = true;
        if (body.data.consent === false)
